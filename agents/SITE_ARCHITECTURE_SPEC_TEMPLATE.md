...
- Repository ignore boundary: local `.bundle/`, root `vendor/bundle/`, root `vendor/cache/`, root `local/`, and `.vscode/` state are ignored; `Gemfile.lock`, `package-lock.json`, and `_sass/vendor/` remain trackable.
