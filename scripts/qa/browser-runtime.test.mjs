...
test("the npm test command does not depend on shell glob expansion", async () => {
  ...
  assert.equal(
    ...
    "node --test scripts/qa/browser-behavior.test.mjs scripts/qa/browser-runtime.test.mjs scripts/qa/build-js.test.mjs scripts/qa/fontawesome-contract.test.mjs scripts/qa/theme-contract.test.mjs && npm run check:js",
  );
});
