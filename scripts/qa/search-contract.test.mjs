...
test("the search manifest contains the intended 209-document subset", async () => {
  ...
  assert.equal(siteRoutes.length, 220);
  assert.equal(searchRoutes.length, 209);
  assert.equal(new Set(searchRoutes).size, 209, "search routes must be unique");
  assert.deepEqual(
