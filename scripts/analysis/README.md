...
- `scripts/analysis/citation_map_parser.R` converts the manually exported Web of Science data in `_data/map.txt` to `_data/map_data.json`. Run it with `Rscript scripts/analysis/citation_map_parser.R`.
- `scripts/analysis/legacy/citation_analysis.R` and `scripts/analysis/legacy/citation_network.R` are retained research prototypes. They are not part of the website build or a supported automated pipeline.
