...
- `npm test` passes 58/58 primary tests, the deterministic JavaScript check, and 26/26 static container-contract tests.
- `npm run check:generators` passes 19/19 CLI tests, including taxonomy failures, malformed/invalid encoding, CRLF normalization, CRLF front matter, case-insensitive duplicate targets, atomic no-clobber behavior, permissions, overwrite preservation, strict validator option parsing, and conflicting-policy rejection.
